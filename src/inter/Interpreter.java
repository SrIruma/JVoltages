...
public class Interpreter implements ExpressionVisitor<Object>, StatementVisitor<Void>{
    ...
    public void interpret(List<Statement> statements) {
        try {
            ...
            voltRuntime.runtimeError(error);
        }
    }
    ...
    public Object visit(PrefixExpression expr) {
        VoltCallable value = (VoltCallable) environment.get(expr.getPrefixOperatorName());
    ...
    public Object visit(InfixExpression expr) {
        VoltCallable value = (VoltCallable) environment.get(expr.getInfixOperatorName());
        List<Object> args = new ArrayList<>();
    ...
    public Object visit(GetExp expr) {
        ...
        if (object instanceof VoltInstance) {
            return ((VoltInstance) object).get(expr.getName());
        }
    ...
    public Object visit(SetExp expr) {
        ...
        Object value = evaluate(expr.getValue());
        ((VoltInstance) object).set(expr.getName(), value);
        return value;
    }
    ...
    public Object visit(SuperExp expr) {
        ...
        VoltClass superclass = (VoltClass) environment.getAt(distance, "super");
        VoltInstance object = (VoltInstance) environment.getAt(distance - 1, "this");
        VoltFunction method = superclass.findMethod(expr.getMethod().lexeme);
    ...
    public Void visit(ClassStatement statement) {
        ...
        if (statement.getSuperClass() != null) {
            ...
            if (!(superclass instanceof VoltClass)) {
        ...
        Map<String, VoltFunction> methods = new HashMap<>();

        //Bind all method into the class to call them with this leter
        for (FunctionStatement method : statement.getMethods()) {
            VoltFunction function = new VoltFunction(method, environment,
                    method.getName().lexeme.equals("init"));
            methods.put(method.getName().lexeme, function);
        }

        VoltClass voltClass = new VoltClass(statement.getName().lexeme, (VoltClass) superclass, environment, methods);

        if (superclass != null) {
            environment = environment.enclosing;
        }

        environment.assign(statement.getName(), voltClass);
        return null;
    }

    @Override
    public Void visit(NativeFunctionStatement statement) {
        VoltLibrary voltLibrary = new VoltLibrary(statement);
        environment.define(statement.getName().lexeme, voltLibrary);
        return null;
    }
    ...
    public Void visit(FunctionStatement statement) {
        VoltFunction function = new VoltFunction(statement, environment, false);
    ...
    public Void visit(ExtensionStatement statement) {
        VoltClass extensionClass = (VoltClass) environment.get(statement.getClassName());
        ...
        VoltFunction extension = new VoltFunction(functionStatement, extensionClass.getEnvironment(), false);
