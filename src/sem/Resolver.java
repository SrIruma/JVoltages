...
public class Resolver implements ExpressionVisitor<Void>, StatementVisitor<Void> {
    ...
    public Void visit(ThisExp expr) {
        if (currentClass == ClassType.NONE) {
            voltRuntime.error(expr.getKeyword(), "Cannot use 'this' outside of a class.");
            return null;
        }
    ...
    public Void visit(Variable expr) {
        if (!scopes.isEmpty() &&
            ...
            voltRuntime.error(expr.getName(), "Cannot read local variable in its own initializer.");
        }
        resolveLocal(expr, expr.getName());
        return null;
    }
    ...
    public Void visit(BreakStatement statement) {
        ...
        if (currentScopeType == MoveKeyword.ScopeType.NONE) {
            voltRuntime.error(statement.getKeyword(), "Continue can only used be inside loops.");
        }
        return null;
    }
    ...
    public Void visit(ContinueStatement statement) {
        // add this case inside semantic to make sure it inside loop
        if (currentScopeType == MoveKeyword.ScopeType.NONE) {
            voltRuntime.error(statement.getKeyword(), "Break can only used be inside loops.");
        }
        return null;
    }
    ...
    public Void visit(ReturnStatement statement) {
        ...
        if (currentFunction == FunctionType.NONE) {
            voltRuntime.error(statement.getKeyword(), "Cannot return from top-level code.");
        }
        if (statement.getValue() != null) {
            if (currentFunction == FunctionType.INITIALIZER) {
                voltRuntime.error(statement.getKeyword(), "Cannot return a value from an initializer.");
            }
            resolve(statement.getValue());
        }
        return null;
    ...
    public Void visit(ClassStatement statement) {
        ...
        if (statement.getSuperClass() != null &&
            ...
            voltRuntime.error(statement.getSuperClass().getName(), "A class cannot inherit from itself.");
        }
    ...
    public Void visit(SuperExp expr) {
        if (currentClass == ClassType.NONE) {
            voltRuntime.error(expr.getKeyword(), "Cannot use 'super' outside of a class.");
        } else if (currentClass != ClassType.SUBCLASS) {
            voltRuntime.error(expr.getKeyword(), "Cannot use 'super' in a class with no superclass.");
        }
        resolveLocal(expr, expr.getKeyword());
        return null;
    }
    ...
    private void declare(Token name) {
        ...
        if (scope.containsKey(name.lexeme)) {
            voltRuntime.error(name, "Variable with this name already declared in this scope.");
        }
